/*
  # OAuth Email Callback Handler Functions

  1. New Functions
    - `initialize_user_onboarding` - Ensures onboarding records exist
    - `update_onboarding_step` - Updates specific onboarding steps
    - `handle_oauth_email_callback` - Processes OAuth email callbacks
    - `get_user_onboarding_status` - Retrieves onboarding status
    - `user_has_connected_emails` - Checks for active email accounts
    - `sync_onboarding_status` - Syncs onboarding with actual data

  2. Security
    - All functions use SECURITY DEFINER for proper access control
    - Input validation and error handling included
    - Proper JSON response format for frontend integration
*/
...
DROP FUNCTION IF EXISTS update_onboarding_step(uuid, text, boolean);
DROP FUNCTION IF EXISTS handle_oauth_email_callback(uuid, text, date);
DROP FUNCTION IF EXISTS get_user_onboarding_status(uuid);
DROP FUNCTION IF EXISTS user_has_connected_emails(uuid);
DROP FUNCTION IF EXISTS sync_onboarding_status(uuid);
