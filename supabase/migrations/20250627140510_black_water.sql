/*
  ...
  1. New Tables
    - `user_email_accounts`
      ...
      - `email` (text, not null)
      - `provider` (text, only 'gmail' allowed)
...
-- Function to add email account (Gmail only)
...
BEGIN
  -- Ensure only Gmail is allowed
  IF email_provider != 'gmail' THEN
    RAISE EXCEPTION 'Only Gmail accounts are supported';
  END IF;

  INSERT INTO user_email_accounts (user_id, email, provider)
  VALUES (user_uuid, email_address, 'gmail')
...
EXCEPTION
  WHEN unique_violation THEN
    RAISE EXCEPTION 'Gmail account already exists for this user';
  WHEN OTHERS THEN
    RAISE EXCEPTION 'Failed to add Gmail account: %', SQLERRM;
END;
